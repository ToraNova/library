...
# Constant var defines
const_LOGS_DIR = 'logs'
const_LOGS_LIM = 100000
...
def attach_Handler(a_loggername,a_handler):
	'''attaches a handler to a logger'''
...
def gen_RotatingFileLogger(a_loggername,a_path):
	'''generates a rotating file logger with an attached handler'''
	gen_handler = RotatingFileHandler(a_path, maxBytes=const_LOGS_LIM, backupCount=1)
	return attach_Handler(a_loggername,gen_handler)

def gen_FileLogger(a_loggername,a_path):
	'''generates a file logger with an attached handler'''
...
srvlog = {
		userlog:gen_FileLogger('user_logger',os.path.join(const_LOGS_DIR,logtofile[userlog])),
		systlog:gen_RotatingFileLogger('sys_logger',os.path.join(const_LOGS_DIR,logtofile[systlog])),
		operlog:gen_RotatingFileLogger('ope_logger',os.path.join(const_LOGS_DIR,logtofile[operlog]))
		}

# Usage examples
# srvlog["sys"].info(useradd_form.username.data+" registered as new user, admin="+str(useradd_form.adminPriv.data))
# srvlog["oper"].warning(<string>)
# srvlog["user"].error(<err_string>)
